...
class AccountContract : Contract {
    ...
    */
    ...
    // validando a conta para pagamento do boleto
    private fun verifyPaymentSlipAccount(tx: LedgerTransaction){

        requireThat {
            "Para pagamento de boleto deve consumir uma conta e o próprio boleto" using (tx.inputs.size == 2)
            "Para pagamento de boleto deve gerar apenas uma nova conta" using (tx.outputs.size == 1)
        }
